...
React application. **Not yet initialized** — if `docs/overview.md` does not
...
- Before declaring any task complete: lint, typecheck, and tests must be
  green, and any UI-affecting change needs a browser smoke test (qa agent,
...
6. Review — adversarial code review by the code-reviewer agent (plus
   security-reviewer for auth/input/data changes) on the full diff; qa runs a
   browser smoke test for UI-affecting changes. Fix findings before landing.
...
- Quality gates: sessions cannot stop with failing lint/typecheck; any
  `git merge`/`git pull` triggers a full lint+build+test gate.
...
| frontend | Components, routing, styling | UI implementation |
| state | Stores, API client, data fetching | Data layer implementation |
...
| devops | CI/CD, build, deploy | Infrastructure work |
...
Launch independent agents in parallel; give implementers their own worktrees.

## Security Quick Rules

Before any PR: no hardcoded secrets; all user input validated; XSS prevention
(no unsanitized `dangerouslySetInnerHTML`); auth checks on protected routes;
error messages don't leak internals. If a security issue is found: STOP →
security-reviewer agent → fix CRITICAL issues → rotate any exposed secrets.
